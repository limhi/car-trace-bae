...
public class PhoneRegAction extends ActionBaseImp {
  private static final Logger logger = Logger.getLogger(PhoneRegAction.class);
  ...
  protected Response merge() throws Exception {
    ...
    if (count == 0) {
      ...
      service.insert(bean);
      // log.info("新增一筆手機註冊記錄：" + JSON.toJSONString(retCR));
    } else if (count == 1) {
      ...
      // log.info("修改一筆手機註冊記錄：" + JSON.toJSONString(retCR));
    } else {
      // log.info("手機記錄數目異常，不處理");
      throw new Exception("手機記錄數目異常，不處理");
    }
